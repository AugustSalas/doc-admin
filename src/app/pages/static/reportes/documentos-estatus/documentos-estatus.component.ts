...
@Component({
  selector: 'app-documentos-estatus',
  templateUrl: './documentos-estatus.component.html',
  styleUrls: ['./documentos-estatus.component.scss']
})
export class DocumentosEstatusComponent implements OnInit {
  ...
  Pdf() { // metodo para generar el pdf de vista previa
   ...
   /* this.dinamicoService.getListado("/webresources/documentofisico/reporte/documentosabiertos/" +
      fechaInicial + "/" + fechaFinal).subscribe(resp => {

        if (resp.length > 0) {

          var lista: DocumentoFisicoReporteMdl = new DocumentoFisicoReporteMdl(resp, fechaInicial, fechaFinal);

          peticion = this.dinamicoService.imprimirReporte("/webresources/documentofisico/docabiertos" , lista);
