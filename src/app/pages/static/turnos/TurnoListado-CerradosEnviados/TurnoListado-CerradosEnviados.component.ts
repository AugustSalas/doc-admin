...
import { TurnosCEVisorComponent } from '../TurnosCerradosEnviados-Visor/TurnosCE-visor.component';

@Component({
  selector: 'app-turnos-list-cerradoRecibidos',
  templateUrl: './Turnolistado-CerradosEnviados.component.html',
  styleUrls: ['./Turnolistado-CerradosEnviados.component.scss']
})
export class TurnosListCerradosEnviadosComponent implements OnInit {
 ...
 //   this.TIservice.TrazaInternaFindallCerradosEnviadosRange(numeroInicio, numeroFinal - 1)  //paginador comentado
 this.TIservice.TrazaInternaFindallCerradosEnviados()   
 .subscribe(resp => {
        this.turnos = resp
  ...
  visorDocumentos(data) {
    ...
    if (data.documentoId != null) {
      this.router.navigate(['/turnos/visorCE/documento/', data.documentoId.documentoId]);
    }
    else {
      //this.router.navigate(['/turnos/visorCE/documentoFisico/', data.documentoFisicoId.documentoFisicoId]);
      const dialogRef = this.dialog.open(TurnosCEVisorComponent, {
        width: '50%',
        height: '90%',
